...
namespace IcuParser\Cli\SelfUpdate;
...
class SelfUpdater
{
    public function run(Output $output): void
    {
        ...
        $output->write('  '.$output->badge('PASS', Output::WHITE, Output::BG_GREEN).' '.$output->success('IcuParser updated successfully.')."\n");
    }
    ...
    {
        return 'https://github.com/yoeunes/icu-parser/releases/latest/download/icu.phar';
    }
    ...
    {
        $tempBase = tempnam(sys_get_temp_dir(), 'icu-phar-');
        if (false === $tempBase) {
    ...
    {
        $tempFile = tempnam(sys_get_temp_dir(), 'icu-update-');
        if (false === $tempFile) {
            throw new \RuntimeException('Unable to create a temporary file.');
        }
